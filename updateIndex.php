...
$doc->loadHTMLFile('/var/www/staging.phippsy.tech/ndismate/app/dist/index.html');
...
if($linkTag || $scriptTag){
    ...
    $version  =date('j M Y - g:ia', time());
    $metaTag->setAttribute('content', $version);

    $doc->saveHTMLFile('/var/www/staging.phippsy.tech/ndismate/app/dist/index.html');
    $init = json_encode([
        "script"=>$scriptSrc,
        "style"=>$linkHref,
        "version"=>$version,
    ]);
    file_put_contents("/var/www/staging.phippsy.tech/ndismate/src/init.json", $init);
}
