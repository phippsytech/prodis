<script>
    ...
    onMount(async () => {
        jspa("/User/Staff", "listDeniedUserStaffByUserId", {
            user_id: user_id,
        }).then((result) => {
...
<div class="bg-white px-3 pt-2 pb-4 mb-2 border border-indigo-100 rounded-md">
    <h1 class="text-slate-800 text-1xl font-bold mt-0">
        Deny access to the following staffs
    </h1>
    <div class="text-sm mb-2 text-gray-500">
        <!-- Denying access to a staff overrides any access granted to this user. -->
        Adding a staff to this list overrides any access previously granted
        to this user.
    </div>
        ...
        >
            {#each staffs as staff, index (index)}
                <li
                    ...
                    index
                        ? 'rounded-b-lg'
                        : ''}border-b border-gray-200 w-full {staff.archived ==
                    1
