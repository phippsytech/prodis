...
<Container>
    ...
    <Container>
        <h1 class="mt-0 mb-2">
            Did Logan recieve his medications by 1 of the staff on this shift?
        </h1>
        <RadioButtonGroup
            options={yes_no_options}
            bind:value={form.form_data.medications}
            {readOnly}
        />
        {#if form.form_data.medications == "no"}
    ...
    <Container>
        <h1 class="mt-0 mb-2">
            When the second person signed off at 10pm was the external gate
            locked?
        </h1>
        <RadioButtonGroup
            options={yes_no_options}
            bind:value={form.form_data.external_gate_locked_at_night}
            {readOnly}
        />
        {#if form.form_data.external_gate_locked_at_night == "no"}
            <p class="mt-2">Provide details why this did not happen.</p>
            <textarea
                placeholder="details here..."
                class="rounded border py-0 p-1.5 border-gray-300 focus:border-blue-600 focus:outline-none w-full"
                style="height:70px"
                bind:value={form.form_data
                    .external_gate_not_locked_at_night_details}
                {readOnly}
            ></textarea>
        {/if}
    </Container>
    ...
    <Container>
        <h1 class="mt-0 mb-2">
            During school drop off today did Logan have to wait in the car when
            he got to school before ringing the doorbell?
        </h1>
        <RadioButtonGroup
            options={yes_no_options}
            bind:value={form.form_data.school_drop_off_wait_in_car}
            {readOnly}
        />
        {#if form.form_data.school_drop_off_wait_in_car == "no"}
            <p class="mt-2">Provide details why this did not happen.</p>
            <textarea
                placeholder="details here..."
                class="rounded border py-0 p-1.5 border-gray-300 focus:border-blue-600 focus:outline-none w-full"
                style="height:70px"
                bind:value={form.form_data.school_drop_off_wait_in_car_details}
                {readOnly}
            ></textarea>
        {/if}
    </Container>
...
<Container>
    ...
    <Container>
        <h1 class="mt-0 mb-2">
            Did you use a 1 person restraint at all with Logan?
        </h1>
        <RadioButtonGroup
            options={yes_no_options}
            bind:value={form.form_data.prn}
            {readOnly}
        />

        {#if form.form_data.prn == "yes"}
            <p class="mt-2">
            ...
            <textarea
                ...
                class="rounded border py-0 p-1.5 border-gray-300 focus:border-blue-600 focus:outline-none w-full"
                style="height:70px"
                bind:value={form.form_data.prn_details}
                {readOnly}
            ></textarea>
        {/if}
    </Container>
    ...
    <Container>
        <h1 class="mt-0 mb-2">
            Did both staff go to the staff room to become safe from Logan during
            a behaviour of concern?
        </h1>
        <RadioButtonGroup
            options={yes_no_options}
            bind:value={form.form_data.safe_area_used}
            {readOnly}
        />

        {#if form.form_data.safe_area_used == "yes"}
            <p class="mt-2">Provide details .</p>
            <textarea
                placeholder="details here..."
                class="rounded border py-0 p-1.5 border-gray-300 focus:border-blue-600 focus:outline-none w-full"
                style="height:70px"
                bind:value={form.form_data.safe_area_used_details}
                {readOnly}
            ></textarea>
        {/if}
    </Container>
