<script>
  ...
  function getLeaveTypeName(leave_type_id) {
    let leave_type = leave.find((item) => item.LeaveTypeID == leave_type_id);
  ...
  function listLeaveApplications() {
    ...
    jspa("/Payroll/Leave", "listLeaveApplications", {}).then((result) => {
      ...
      scheduled_leave_applications = leave_applications.filter(
        (item) => item.LeavePeriods[0].leave_period_status == "SCHEDULED"
      );

      processed_leave_applications = leave_applications.filter(
        (item) => item.LeavePeriods[0].leave_period_status == "PROCESSED"
      );
...
<div class=" mb-4">
  ...
  <div class="">
    This page contains approved leave requests that will be processed in the
    appropraite pay period.
  </div>
</div>
...
<Container>
  ...
  {#each scheduled_leave_applications as item, index}
    <div
      ...
      scheduled_leave_applications.length - 1
        ? 'border-b border-b-indigo-100'
        : ''} lg:px-8"
    >
      <div class="text-2xl text-gray-900 w-20 text-center">
        <ScaledNumberDisplay value={item.LeavePeriods[0].number_of_units} />
      ...
      <div>
        ...
        >
        :
        <span class="font-light">{getLeaveTypeName(item.leave_type_id)}</span>
        <div class="text-sm font-medium text-gray-900">
          {formatDate(item.start_date)}{#if item.start_date != item.end_date}
