<script>
  ...
  jspa("/SignatureRequest", "listSignatureRequests", {}).then((result) => {
    ...
    const filteredRequests = result.result.filter(
      (item) => item.most_recent_event !== "signed"
    );
    signature_requests.set(filteredRequests);
  });

  BreadcrumbStore.set({
    path: [{ name: "Service Agreements" }],
  });
...
<!-- <div class="flex items-center mb-4">
  <div class="flex-auto">
    <h1 class="text-2xl font-fredoka-one-regular px-4" style="color:#220055;">
      Signature Requests
    </h1>
  </div>
</div> -->

<!-- {signature_requests.length} Signature Requests -->
      ...
      <div class="staff-member">
        <div class="text-sm font-bold">
          <a
            ...
            href="/#/serviceagreements/pending/{request.id}">{request.title}</a
          >
        </div>
        <div class="text-xs">
          #{request.id}
