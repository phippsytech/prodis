<script>
    ...
    async function loadTaskList() {
        jspa("/Task", "listTasks", {staff_id: stafferStore.id})
			.then((result) => {
                tasks = result.result;
            },
        );
    ...
    $: {
        taskList = tasks.filter(
            (task) =>
            task.title.toLowerCase().includes(search.toLowerCase()) == true,
        );
    }
...
<div class="bg-white rounded-lg border border-indigo-100 w-full text-slate-800 p-4">
    ...
    <table class="w-full">
        ...
        {#each taskList as task, index (index)}
            <tr 
                class="border-b hover:bg-blue-700 hover:text-white focus:outline-none focus:ring-0 focus:bg-gray-200 focus:text-gray-600 transition duration-500 cursor-pointer {isOverDue(task.due_date) ? 'bg-red-500 text-white' : ''}"
