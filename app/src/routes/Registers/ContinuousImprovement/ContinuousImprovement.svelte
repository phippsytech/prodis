<script>
  ...
  onMount(() => {
    if (params.id != "add") {
      jspa("/Register/ContinuousImprovement", "getContinuousImprovement", {
        id: params.id,
      })
        .then((result) => {
  ...
  $: {
    if (mounted) {
      const valueChanged =
        JSON.stringify(continuous_improvement) !== JSON.stringify(stored_value);
  ...
  $: continuous_improvement.implementing_staffs_id =
  ...
  const validations = [
    {
      check: () =>
        !continuous_improvement.involved_staffs_id ||
        continuous_improvement.involved_staffs_id.length === 0,
    ...
    {
      check: () =>
        continuous_improvement.review_date &&
        !continuous_improvement.implementing_staffs_id,
  ...
  function save() {
    if (validate()) {
      ...
      continuous_improvement.implementation_date =
        continuous_improvement.action_taken &&
        continuous_improvement.review_date &&
        continuous_improvement.implementing_staffs_id
      ...
      continuous_improvement.status =
        continuous_improvement.review_date &&
        continuous_improvement.impact_analysis
          ? ((continuous_improvement.completion_date =
              continuous_improvement.completion_date || current_date),
            "completed")
          : continuous_improvement.status;

      jspa("/Register/ContinuousImprovement", "updateContinuousImprovement", {
        ...
        .catch(() => {
          toastError(
            "Error updating continuous improvement, please try again."
          );
        });
    }
  }
  ...
  function deleteContinuousImprovement() {
    if (
      confirm("Are you sure you want to delete this continuous improvement?")
    ) {
      jspa("/Register/ContinuousImprovement", "deleteContinuousImprovement", {
        id: continuous_improvement.id,
      })
        .then((result) => {
...
<ContinuousImprovementForm bind:continuous_improvement bind:staffer />
