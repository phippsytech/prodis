<script>
  ...
  const validations = [
    {
      check: () => !conflictofinterest.date_identified,
      message: "Conflict of Interest date must be provided.",
    },
    {
      check: () => !conflictofinterest.type,
      message: "Conflict of Interest type must be provided.",
    },
    {
      check: () => !conflictofinterest.status,
      message: "Conflict of Interest status must be provided.",
    },
    // { check: () => !conflictofinterest.staff_id , message: "Please select a staff." },
    {
      check: () => !conflictofinterest.parties_involved,
      message: "Parties Involved name must be provided.",
    },
    {
      check: () => !conflictofinterest.description,
      message: "Details must be provided.",
    },
    {
      check: () =>
        conflictofinterest.date_identified > conflictofinterest.date_resolved &&
        conflictofinterest.resolution.trim() !== "",
      message: "Resolution date must not be before the conflict date.",
    },
    {
      check: () =>
        conflictofinterest.date_resolved && !conflictofinterest.resolution,
      message: "Resolution taken must be provided.",
    },
  ];
    ...
    if (params.id != "add") {
      jspa("/Register/ConflictOfInterest", "getConflictOfInterest", {
        ...
        .finally(() => {
          ...
          stored_conflictofinterest = Object.assign({}, conflictofinterest);
        });
    }
    mounted = true;
  });
  ...
  $: conflictofinterest.date_resolved = conflictofinterest.resolution
    ...
    : conflictofinterest.date_resolved;
  ...
  function deleteConflictOfInterest() {
    if (confirm("Are you sure you want to delete this conflict of interest?")) {
      jspa("/Register/ConflictOfInterest", "deleteConflictOfInterest", {
  ...
  $: {
    ...
    if (
      stored_conflictofinterest.date_resolved &&
      stored_conflictofinterest.resolution.trim() != ""
    ) {
      conflictofinterest.status = "resolved";
    }
  }
