<script>
  ...
  function handleFilters() {
    ...
    if (staff_id) {
      filteredManaged = filteredManaged.filter(
        (item) => item.StaffId == staff_id
      );
    }

    if (planmanager_id) {
      filteredManaged = filteredManaged.filter(
        (item) => item.PlanManagerId == planmanager_id
      );
    }

    if (start_date) {
      filteredManaged = filteredManaged.filter(
        (item) => item.SupportsDeliveredFrom >= start_date
      );
    }

    if (end_date) {
      filteredManaged = filteredManaged.filter(
        (item) => item.SupportsDeliveredFrom <= end_date
      );
    }

    selectedLineItems = selectedLineItems.filter((selectedItem) =>
      filteredManaged.some(
        (managedItem) => managedItem.SessionId === selectedItem
      )
    );
  }
</script>
        ...
        <div class="flex">
          ...
          <div class="ml-3 flex-1 md:flex md:justify-between">
            <p class="text-sm text-blue-700">
              <span class="font-bold">No billables to display.</span> Filters may
              be restricting the results.
            </p>
          </div>
        </div>
      </div>
