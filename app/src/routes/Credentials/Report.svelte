  ...
  <div class="mb-2">
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class=" text-sm text-gray-700">
          Here are the required credentials for the selected staff as of {formattedDate}.
        </p>
      </div>
      ...
      {#each credentials as credential, index (index)}
        <tr
          ...
          class={credential.credential_status === "Missing"
            ...
            : ""}
        >
          <td class="p-2 text-left font-medium">{credential.staff_name}</td>
              ...
              <div>
                <div class="font-semibold">
                  {#if credential.file_reference}
                    <button
                      on:click={() => {
                        openFile(credential.file_reference);
                      }}
                    >
                ...
                {#if credential.expiry_date && isDateWithinSixWeeks(credential.expiry_date)}
                  <div class="text-xs">
                    Renew by {formatDate(credential.expiry_date)} ({getDaysUntilDate(
                ...
                {#if credential.expiry_date && hasDatePassed(credential.expiry_date)}
                  <div class="text-xs">
                    ...
                    ({getDaysUntilDate(credential.expiry_date)})
                  </div>
                {/if}
              </div>
