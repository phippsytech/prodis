<script>
    ...
    function removeTeamMember(staff_id) {
    ...
    onMount(async () => {
        jspa("/Staff/Team", "listStaffBySupervisorId", {
            supervisor_id: supervisor_id,
        }).then((result) => {
            staff = result.result;
        });
...
<div class="bg-white px-3 pt-2 pb-4 mb-2 border border-gray-300 rounded-md">
    <h1 class="text-black text-1xl font-bold mt-0 mb-2">Supervises</h1>
