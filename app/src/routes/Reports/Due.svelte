    ...
    reportList = reports
      .filter((report) => {
        const typeFilter = filters.some(
          (filter) => filter.label === report.report_type && filter.enabled
        );
        const holdFilter = !(
          report.on_hold &&
          !filters.some(
            (filter) => filter.label === "on hold" && filter.enabled
          )
        );
        const staffFilter = staff_id == null || report.staff_id == staff_id;
        return report.report_type && typeFilter && holdFilter && staffFilter;
      })
...
<div class="bg-white px-3 mb-2 rounded-md pb-1">
  ...
  <div class="flex items-center">
    ...
    <div>
      ...
      {:else}<span class="text-sm text-slate-500">Filter by Staff Member</span
        >{/if}
    </div>
  </div>
</div>
