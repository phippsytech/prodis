<script>
    ...
    function calculateTotal(key) {
        return clients.reduce((total, client) => total + parseFloat(client[key]), 0);
    }
...
<JSON2CSV filename="participantbudgets.csv" bind:json_data={clients} label="Export results to CSV" />
...
{#each clients as client }
    ...
    >
        ...
        <div>
            ...
            <span class="text-xs">({getDaysUntilDate(client.ndis_plan_end_date)})</span>
        </div>
        <div>{client.code}</div>
        <div>{formatCurrency(client.rate)}</div>
        <div>{formatCurrency(client.budget)}</div>
        <div>{formatCurrency(client.remaining)}</div>
        <div>{@html convertMinutesToHoursAndMinutes(Math.round((client.remaining / client.rate) * 60))}</div>
    </div>
{/each}
