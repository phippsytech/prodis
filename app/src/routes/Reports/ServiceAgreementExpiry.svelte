<script>
  ...
  document.title = "Client Service Agreements Report";
  BreadcrumbStore.set({
    path: [
      { url: "/reports", name: "Reports" },
      { url: null, name: "Client Service Agreements" },
    ],
  });

  async function getReports(staff_id) {
    let result = await jspa("/Report", "getExpiredServiceAgreements");
...
<div class="px-2 mb-2">
  <h2 class="text-2xl text-indigo-700 tracking-tight font-fredoka-one-regular">
    Client Service Agreements Report
  </h2>
  <p>Summary of the status of Service Agreements</p>
</div>
  ...
  {#each reports as report, index (index)}
    ...
    >
      <div class="flex justify-between">
        ...
        <div>
          <span class=""> {formatDate(report.service_agreement_end_date)}</span>
        </div>
      </div>
    </li>
  {:else}
