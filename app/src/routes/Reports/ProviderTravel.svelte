<script>
  ...
  let filters = [
    {
      label: "Speed < 40Kms/hr",
      value: "not_acceptable_speed",
      enabled: false,
    },
  ];
  ...
  $: {
    ...
    if (showNotAcceptableSpeed) {
      trips_list = trips.filter(
        (trip) => !isSpeedAcceptable(trip.kms, trip.trip_duration)
      );
    } else {
      trips_list = trips;
    }
  ...
  function displayClaimableKms(trip) {
    ...
    if (trip.trip_duration > maxClaimableTime) {
      ...
      claimableKms = Math.round(trip.kms * claimablePercentage * 100) / 100; // Rounded to 2 decimal places
...
<div class="bg-slate-100 px-3 pt-2 pb-4 my-2 rounded-md">
  ...
  <div class="flex flex-wrap gap-2 items-center">
    ...
    <div class="sm:flex-none w-full sm:w-auto min-w-[200px]">
      <StaffSelector bind:staff_id clearable />
    </div>
    <div class="sm:flex-none w-full sm:w-auto min-w-[270px]">
      <ClientSelector bind:client_id bind:staff_id clearable />
    </div>
  </div>
</div>
        ...
        >
          <div class="text-sm font-medium">
            <div class="font-semibold">
              ...
              <span class={trip.client_id ? "" : "font-italic text-slate-500"}>
                {trip.staff_name}
              </span>
              ...
              >
              <div>
                {#if trip.client_id}{trip.client_name}{:else}No Participant{/if}
              </div>
            </div>
            <div>
