<script>
    import List from "../../routes/Staff/List.svelte";
    import StaffNavList from "./NavList/StaffNavList.svelte";
    import { jspa } from "@shared/jspa.js";

    const pathname = window.location.hash;
    const segments = pathname.split("/");
    const staff_id = segments[2];

    let staff = {};

    // $: {
    if (staff_id) {
        jspa("/Staff", "getStaffer", { id: staff_id }).then((result) => {
            staff = result.result;
        });
    } else {
        staff = {};
...
>
    <div class="flex h-16 shrink-0 items-center">
        <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-2 px-6">
            ...
            <div class="flex items-center gap-x-4 lg:gap-x-6">
                ...
                >
                    <span class="sr-only">Add staff</span>
                    <svg
                        class="h-6 w-6"
                        fill="none"
                        stroke="currentColor"
    ...
    {#if search.length > 0 || !staff_id}
        <List bind:search />
    {:else if !staff_id}{:else}
            ...
            <li class="relative flex justify-between py-2 text-gray-700">
                <div class="min-w-0 flex-auto">
                    <div class="flex justify-between items-center px-5">
                        ...
                        <a href="/#/staffs/{staff.id}/settings">
                            <svg
